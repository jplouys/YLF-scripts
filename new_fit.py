...
Creation Date: 2020
 Last Modification Date: 2024-12-13 09:46:37
...
def decimales(
    ...
    try:
        ...
        print("Rounding error. OverflowError in x=", x)
    return s
...
def fit(
    ...
    plt.figure(figsize=(16, 12))
    if type(xerr) == type(0.0) or type(xerr) == type(0):
        xerr = np.repeat(xerr, len(x))
    else:
        pass
    if type(yerr) == type(0.0) or type(yerr) == type(0):
    ...
    if nombre_params == []:
        ...
        pass
    if loglog == False:
        if type(x[0]) == type([]) or type(x[0]) == type(
            np.array([])
        ):  # chequea si es solo un set de datos o si no
    ...
    if type(params) != type([]) and silent == False:
        for i in range(len(params)):
            print(f"Parámetro {nombre_params[i]}: {params[i]}", "\u00b1", stds[i])
        print(
...
def Odr(
    ...
    try:
        for i in range(len(std)):
            std[i] = round(
                std[i], -int(math.floor(math.log10(abs(std[i])))) + sigfig - 1
            )  # aca redondea el error a una cifra significativa
            param[i] = round(
                param[i], decimales(std[i])
            )  # redondea el parametro a tantos decimales como tenga el error
    except:
        pass
